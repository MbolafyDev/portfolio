...
if (!isNew) {
  const { data, error } = await useFetch(`/api/admin/education/${id}`)
  if (error.value) {
    throw createError({ statusCode: 404, statusMessage: "Parcours introuvable" })
  }
  if (data.value) {
    const it: any = data.value
    form.school = it.school || ""
    form.degree = it.degree || ""
    form.test = it.test || ""
...
async function save() {
  ...
  try {
    ...
    msg.value = "✅ Formation mise à jour"
  } catch (e: any) {
...
useHead({
  title: isNew ? "Nouvelle formation | Admin" : `Formation #${id} | Admin`,
})
</script>
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-3xl font-extrabold text-blue-950">
          Admin · Formations · {{ isNew ? "Nouveau" : "Modifier" }}
        </h1>
        <p class="mt-2 text-blue-950/70">
          {{ isNew ? "Ajoute une formation." : "Modifie ta formation." }}
        </p>
      </div>
