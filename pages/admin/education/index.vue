...
const { data: items, refresh } = await useFetch("/api/admin/education")
...
<template>
  <main class="max-w-4xl mx-auto px-4 py-10">
    <div class="flex items-start justify-between gap-4">
      <div>
        <h1 class="text-3xl font-extrabold text-blue-950">Admin · Formations</h1>
        <p class="mt-2 text-blue-950/70">Gère ton parcours académique.</p>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <NuxtLink to="/admin/education/new" class="rounded-xl bg-blue-950 px-4 py-2 font-bold text-white">
          + Nouveau
        </NuxtLink>
      </div>
    </div>
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <div class="mt-1 text-blue-950/80">{{ it.degree }}</div>
            <div v-if="it.test" class="mt-1 text-blue-950/70">{{ it.test }}</div>

            <div class="mt-3">
              <span class="inline-flex rounded-full border border-orange-200 bg-amber-50 px-3 py-1 text-xs font-bold text-orange-700">
                {{ it.period }}
              </span>
            </div>
          </div>
